resource "aws_cloudwatch_event_rule" "supplier_to_core" {
  name           = "${local.csi}-supplier-to-core"
  description    = "Event rule for inbound supplier events to forward to Core"
...
resource "aws_cloudwatch_event_target" "suppliers_queue" {
  count = var.event_target_arns["notify_core_sns_topic"] != null ? 1 : 0

  rule           = aws_cloudwatch_event_rule.supplier_to_core.name
  arn            = var.event_target_arns["notify_core_sns_topic"]
  target_id      = "notify-core-suppliers-queue"
  event_bus_name = aws_cloudwatch_event_bus.data_plane.name

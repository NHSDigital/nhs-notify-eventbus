module "core_to_supplier_api_events_dlq" {
  ...
  project         = var.project
  region          = var.region
  name            = "core-to-supplier-api-events-dlq"
  sqs_kms_key_arn = module.kms.key_arn

  sqs_policy_overload = data.aws_iam_policy_document.core_to_supplier_api_events_allow_eventbridge[0].json
}
...
data "aws_iam_policy_document" "core_to_supplier_api_events_allow_eventbridge" {
  ...
  statement {
    ...
    resources = [module.core_to_supplier_api_events_dlq[0].sqs_queue_arn]
    condition {
      test     = "ArnEquals"
      variable = "aws:SourceArn"
      values   = [aws_cloudwatch_event_rule.core_to_supplier_api[0].arn]
    }
  }
}
